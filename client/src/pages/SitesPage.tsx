    ...
    <div className="p-4 md:p-8 lg:p-8 space-y-6 min-h-full font-sans">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <button
          ...
          className="clay-btn inline-flex items-center gap-2 px-4 py-2.5 cursor-pointer"
        >
      ...
      {isLoading ? (
        ...
        <div className="clay-card p-12 text-center text-slate-400">No active construction sites registered.</div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {projects.map((proj: any) => (
            <motion.div
              ...
              className="clay-card p-6 space-y-5 flex flex-col justify-between"
            >
              <div className="space-y-3">
                ...
                <Link to={`/projects/${proj.id}`} className="block hover:underline">
                  <h3 className="text-xl font-bold text-slate-800 font-heading">{proj.name}</h3>
                </Link>
              ...
              <div className="pt-4 border-t border-violet-100/30 space-y-3">
                <div className="space-y-1">
                  <div className="flex justify-between text-xs font-semibold">
                    <span className="text-slate-600">Physical Slab Completion</span>
                    <span className="text-[#7C6EF0] font-mono">{proj.progress || 0}%</span>
                  </div>
                  <div className="h-2.5 w-full bg-slate-100 rounded-full overflow-hidden">
                    <div className="h-full bg-clay-violet rounded-full" style={{ width: `${proj.progress || 0}%` }} />
                  </div>
                </div>
      ...
      {isAddOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/40 backdrop-blur-sm">
          <div className="clay-card w-full max-w-lg p-6 space-y-6 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between border-b border-violet-100/30 pb-4">
              <h3 className="font-bold text-lg font-heading text-slate-800">Add New Construction Site</h3>
            ...
            <form onSubmit={handleCreate} className="space-y-4 text-sm">
              <div className="grid grid-cols-2 gap-4">
                ...
                <input
                  ...
                  className="w-full clay-input font-mono uppercase"
                />
              </div>
              <select
                required
              ...
              <textarea
                ...
                className="w-full clay-input h-20"
              />
              <div className="flex justify-end gap-3 pt-4 border-t border-violet-100/30">
                ...
                <button type="submit" disabled={createMutation.isPending} className="clay-btn px-5 py-2">Register Site</button>
              </div>
            </form>
          </div>
        </div>
            ...
            <form onSubmit={handleUpdate} className="space-y-4 text-sm">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full clay-input font-semibold"
                >
              ...
              <div className="flex justify-end gap-3 pt-4 border-t border-violet-100/30">
                <button type="button" onClick={() => setEditingSite(null)} className="px-4 py-2 rounded-xl text-slate-600 text-sm font-semibold hover:bg-slate-100">Cancel</button>
